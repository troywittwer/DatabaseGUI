...
public class Main extends Application implements EventHandler<ActionEvent> {
  ...
  /**
   * Basic components of the initial GUI turn-in. queryBox is a TextField that the user can type a
  ...
  public void start(Stage primaryStage) throws Exception {
    ...
    try {
      ...
      queryBox = new TextField(DEFAULT_QUERY); // populate the text field with the default query
      ...
      primaryStage.setScene(new Scene(myGrid, 300, 275)); // add GridPane to scene
      primaryStage.show();
  ...
  public void handle(ActionEvent event) {
    if (event.getSource() == queryButton) { // User hit the query button.
      try {
        ...
        try { // sets the TextField back to default text
